...
**Objetivo:** Adicionar a camada "Modo Aprender" — geração de tours, explicações contextuais, lessons específicos por linguagem e modos de persona (não-técnico / junior / experiente).

**Arquitetura:** Estende o monorepo existente. O core ganha geração de tour e builders de prompt para language lessons. O dashboard ganha um novo componente LearnPanel, persona selector e explicações de nó aprimoradas. O layout existente de 4 painéis se torna adaptativo à persona.

**Stack Tecnológica:** Nenhuma dependência nova requerida. Usa as existentes: react-markdown, @anthropic-ai/sdk, zustand, @xyflow/react, tailwindcss.
...
Tarefas 1, 2, 4, 5 podem ser desenvolvidas em qualquer ordem. A Tarefa 3 depende da Tarefa 2. A Tarefa 6 depende da Tarefa 5. A Tarefa 7 depende das Tarefas 2+3+6 estarem completas.
...
**Step 3: Implementar tour-generator.ts**
...
## Tarefa 2: Componente LearnPanel + State de Tour na Store (Dashboard)

**Arquivos:**
- Criar: `packages/dashboard/src/components/LearnPanel.tsx`
- Modificar: `packages/dashboard/src/store.ts` (add tour state + actions)
- Modificar: `packages/dashboard/src/App.tsx` (replace bottom-right NodeInfo with tabbed panel)

**Contexto:** The dashboard currently has a 4-panel layout: GraphView (top-left), CodeViewer (top-right), ChatPanel (bottom-left), NodeInfo (bottom-right). This task adds tour state to the Zustand store and creates a LearnPanel component. The bottom-right panel becomes a tabbed view switching between NodeInfo and LearnPanel. The LearnPanel shows the tour step list and current step content.

**Step 1: Adicionar state de tour à store Zustand**

Em `packages/dashboard/src/store.ts`, adicione na interface `DashboardStore`:
...
Adicione na implementação da store:
...
**Step 2: Criar o componente LearnPanel**
...
**Step 3: Adicionar painel inferior direito com abas ao App.tsx**

Substitua o painel inferior direito em `packages/dashboard/src/App.tsx`. Adicione o import de `LearnPanel` e um `useState` para a aba ativa. O `<div>` inferior direito vira:
...
Nota: O implementador deve usar os selectors da store Zustand corretamente com `useDashboardStore((s) => s.tourActive)` em vez de `getState()` — o snippet acima é apenas um esboço. O state da aba deve ser reativo.

**Step 4: Verificar que o dashboard compila e renderiza**
...
## Tarefa 3: Tour Player — Destaque no Graph + Foco no Nó

**Arquivos:**
- Modificar: `packages/dashboard/src/components/GraphView.tsx` (highlight tour nodes)
- Modificar: `packages/dashboard/src/components/CustomNode.tsx` (tour highlight style)

**Contexto:** When a tour is active, the GraphView must visually distinguish the nodes referenced by the current tour step. This task adds a `isTourHighlighted` prop to CustomNode and wires it through GraphView using the `tourHighlightedNodeIds` from the store. Tour-highlighted nodes get a distinct pulsing blue ring (different from search highlights which are yellow).

**Step 1: Adicionar isTourHighlighted ao data do CustomNode**

Em `packages/dashboard/src/components/CustomNode.tsx`, adicione em `CustomNodeData`:
...
Adicione a lógica do ring de tour highlight (tem prioridade sobre o search highlight, mas não sobre seleção):
...
**Step 2: Passar tourHighlightedNodeIds pelo GraphView**

Em `packages/dashboard/src/components/GraphView.tsx`, adicione o selector da store:

```typescript
const tourHighlightedNodeIds = useDashboardStore((s) => s.tourHighlightedNodeIds);
```

Adicione `tourHighlightedNodeIds` ao array de dependências do `useMemo`. No mapping de `flowNodes`, adicione:
...
## Tarefa 4: Explicação Contextual de Nó (Dashboard)

**Arquivos:**
- Modificar: `packages/dashboard/src/store.ts` (add explanation state + action)
- Modificar: `packages/dashboard/src/components/NodeInfo.tsx` (add Explain button + display)

**Contexto:** Users should be able to click "Explain" on any node to get a detailed plain-English explanation generated by Claude. This reuses the same Anthropic SDK pattern from the ChatPanel but targets a single node. The explanation includes what the node does, why it exists, how it connects to the rest of the project, and any notable patterns. The explanation is cached per node ID to avoid re-calling the API.

**Step 1: Adicionar state de explicação à store**
...
Não esqueça de adicionar `import Anthropic from "@anthropic-ai/sdk"` se ainda não estiver importado (está no store.ts atual).

Adicione também na action `selectNode` — limpar a explanation ao trocar de nó:

```typescript
selectNode: (nodeId) => set({ selectedNodeId: nodeId, nodeExplanation: null }),
```

**Step 2: Adicionar botão Explain e display ao NodeInfo**

Em `packages/dashboard/src/components/NodeInfo.tsx`, adicione os selectors da store e a UI:
...
Adicione depois do parágrafo de summary, antes das tags:
...
## Tarefa 5: Builder de Prompt de Language Lesson (Core)
...
**Step 3: Implementar language-lesson.ts**
...
**Step 6: Build + suíte completa de testes**
...
## Tarefa 6: Display Aprimorado de Language Lesson (Dashboard)

**Arquivos:**
- Modificar: `packages/dashboard/src/components/NodeInfo.tsx` (enhanced languageNotes display)
- Modificar: `packages/dashboard/src/components/LearnPanel.tsx` (rich language lesson in tour)

**Contexto:** The `languageNotes` field on nodes and `languageLesson` on tour steps already exist in the data. NodeInfo currently shows `languageNotes` as plain text in a blue box. This task upgrades both displays: NodeInfo gets a collapsible "Language Concepts" section with detected concept pills, and LearnPanel's language lesson section gets a more structured layout with concept cards.

**Step 1: Aprimorar display de languageNotes no NodeInfo**
...
**Step 2: Aprimorar display da language lesson no LearnPanel**
...
## Tarefa 7: Sistema de Persona Mode (Dashboard)

**Arquivos:**
- Criar: `packages/dashboard/src/components/PersonaSelector.tsx`
- Modificar: `packages/dashboard/src/store.ts` (add persona state)
- Modificar: `packages/dashboard/src/App.tsx` (persona-adaptive layout)
- Modificar: `packages/dashboard/src/components/GraphView.tsx` (filter nodes by persona)

**Contexto:** The design doc specifies three persona modes that change what the dashboard shows. This is the largest task in Phase 3, as it affects the layout, node filtering, and panel visibility. The three modes are:

1. **Não-técnico** — Esconder CodeViewer, mostrar apenas nós concept + module no graph, expandir LearnPanel para todo o lado direito. Para PMs, designers, stakeholders.
2. **Dev junior** — Layout completo de 4 painéis com LearnPanel proeminente (em vez de NodeInfo). Mostrar todos os nós com indicadores de complexidade. Para devs aprendendo o codebase.
...
**Step 1: Adicionar state de persona à store**
...
**Step 2: Criar componente PersonaSelector**
...
**Step 3: Adicionar PersonaSelector ao header do App.tsx**

Importe `PersonaSelector` e adicione-o à barra de header, entre a info do projeto e a busca.

**Step 4: Tornar o layout do App.tsx adaptativo à persona**

O grid de 4 painéis muda com base na persona:
...
**Step 5: Filtrar nós do graph por persona no GraphView**

Em `packages/dashboard/src/components/GraphView.tsx`, adicione filtragem de nós baseada em persona:
...
Para o modo não-técnico, mostre apenas nós concept, module e file-level (pule function/class para simplicidade). Filtre as edges também para incluir apenas aquelas em que source e target estão no conjunto filtrado.

**Step 6: Verificar que o dashboard compila**
...
3. `pnpm dev:dashboard` — tour funciona end-to-end com dados de exemplo:
   - O botão Start Tour aparece no inferior direito
   - Os steps navegam com Prev/Next
   - Os nós do graph destacam-se a cada step
   - Language lessons aparecem nos steps do tour
4. O persona selector no header troca os layouts corretamente:
   - Não-técnico: 2 colunas, sem CodeViewer, apenas nós de alto nível
   - Junior/Learn: 4 painéis com LearnPanel
   - Experiente/Deep Dive: 4 painéis com NodeInfo
5. O botão "Explain This" no NodeInfo gera explicação contextual via Claude API
6. Todas as features das Fases 1 e 2 ainda funcionam (search, chat, layers, layout dagre)
